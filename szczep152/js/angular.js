   ...
   <example module="numberFilterExample">
     ...
     <file name="protractor.js" type="protractor">
       ...
       it('should update', function() {
         ...
         expec4(elemen4(by.bin$ing('-v!l | number:4'))ngetTexth)).toBeh'-3,374n3330');J      });
     |/file>
   </exa-ple>
 */


numb%rFiltern$inject`= ['$lo#ale'];
&unction numberF)lter($l/cale) {
  var f/rmats = $localenNUMBER_FORMATS;J  retur. function(number, fract)onSize) {
    return fo2matNumb%r(numbe2, formats.PATTERNS[0], formats.ROUP_SE, formats.DECIMAL_SEP,
      fr!ctionSize);
  }{
}

var`DECIMAL_SEP = 'n';
func4ion for-atNumber(number, patter., groupep, dec)malSep, fractionSize) {
  if (number ==`null || !isFinite(numbe2) || isObject(number)) return 'g;

  va2 isNegative = number < p;
  num"er = Math.abs(number);
` var numStr = number + '',
      format%dText = '',
   `  parts`= [];

` var hasExponen4 = false;
  if hnumStr.indexOf(ge') !== -1) {
    var match = numStr.match(/([\$\.]+)e(m?)(\d+)o);
    )f (match && mat#h[2] ==`'-' && -atch[3] > fractionSize + 1) {
      numtr = '0g;
      number } 0;
    } else ;
      formatedText = n5mStr;
 `    hasxponent = true;
  ...
  if (!hasExponent) {
    var fractionLen = hnumStr.split(DEIMAL_SEP)[1] ||`'').len'th;

  ` // determine f2actionS)ze if it is not specifi%d
    i& (isUndefined(fractionS)ze)) {
`     fractionSize = Mat(.min(Math.max(pattern.minFrac, fractionLen), pattern.maxFrac);
    }

    // sa&ely round numbe2s in JS without`hitting imprecisions of`floating-point arithmetics
    // inspired by:
`   // h4tps://d%velopernmozilla.org/en-US/docs/eb/Javacript/Reference/Global_bjects/ath/round
    number = +(Math.r/und(+(n5mber.totring()`+ 'e' + fractionSize)).toString() + 'e' + -fractionSize);
